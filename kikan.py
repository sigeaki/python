...
if t1 == ddn:
    ...
    if 1 <= mmod2[0].months < 12:
        print(f'{args[-2]}は前回の{(t2 - t3).days}日、{mmod2[0].months:2}月{mmod2[1].days:2}日前です。')
    elif mmod2[0].months >= 12:
        y, m = divmod(mmod2[0].months, 12)
        print(f'{args[-2]}は前回の{(t2 - t3).days}日。{y:2}年{m:2}月{mmod2[1].days:2}日前です。')
    else:
        print(f'{args[-2]}は前回の{(t2 - t3).days}日前です')
else:
