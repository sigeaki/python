...
if t1 == ddn:
    ...
    # print(mmod2[0].months)
    if 1 <= mmod2[0].months < 12:
        print(f'前回は今日から{(t1 - t2).days}日 {mmod2[0].months:2}月{mmod2[1].days:2}日前です。')
    ...
    if 1 <= mmod3[0].months < 12:
        ...
        print(f'前々回は{t3} 前回の{(t2 - t3).days}日 {y:2}年{m:2}月{mmod3[1].days:2}日前です。')
    else:
    ...
    if 1 <= mmod2[0].months < 12:
        ...
        y, m = divmod(mmod2[0].months, 12)
        print(f'{args[-2]}は前回から{(t1 - t2).days}日。{y:2}年{m:2}月{mmod2[1].days:2}日前です。')
    else:
